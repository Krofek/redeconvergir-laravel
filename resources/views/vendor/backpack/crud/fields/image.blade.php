<div class="form-group col-md-12 image" data-preview="#{{ $field['name'] }}" data-aspectRatio="{{ isset($field['aspect_ratio']) ? $field['aspect_ratio'] : 0 }}" data-crop="{{ isset($field['crop']) ? $field['crop'] : false }}" @include('crud::inc.field_wrapper_attributes')>
    ...
    <div class="row">
        <div class="col-sm-6" style="">
            @php
                $imgSrc = old($field['name']) ? old($field['name']) : (isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : '') );
                if(isset($imgSrc) && $imgSrc != ''){
                    $imgSrc = url($imgSrc);
                }
                //dd($field);
            @endphp
            <img id="mainImage" data-placeholder="{{ $field['placeholder'] }}" src="{{ $imgSrc }}">
        </div>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('.form-group.image').each(function(index){
                ...
                var $previewLg = $(this).find('.img-preview')
                ...
                if (!$mainImage.attr('src')){
                    $remove.hide();
                    $mainImage.attr('src', $mainImage.data('placeholder'));
                    $previewLg.hide();
                    console.log($previewLg);
                }
